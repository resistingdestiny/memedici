...
function getAccounts(networkName) {
  ...
  const hederaPrivateKey = process.env.HEDERA_PRIVATE_KEY || process.env.PRIVATE_KEY;
  
  if (networkName.includes('hedera')) {
    return hederaPrivateKey ? [hederaPrivateKey] : [];
  }
  
  return defaultPrivateKey ? [defaultPrivateKey] : [];
}
...
module.exports = {
  ...
  networks: {
    ...
    hederaTestnet: {
      ...
      accounts: getAccounts('hederaTestnet'),
      chainId: 296,
      gasPrice: 530000000000, // 530 Gwei - Hedera minimum requirement
      timeout: 60000,
    },
