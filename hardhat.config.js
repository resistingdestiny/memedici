...
// Helper function to get accounts for different networks
function getAccounts(networkName) {
  // Default private key (Flow, Rootstock, etc.)
  const defaultPrivateKey = process.env.PRIVATE_KEY;
  
  // Hedera uses different private key
  let hederaPrivateKey = process.env.HEDERA_PRIVATE_KEY || process.env.PRIVATE_KEY;
  if (hederaPrivateKey && !hederaPrivateKey.startsWith("0x")) {
    hederaPrivateKey = "0x" + hederaPrivateKey;
  }

  if (networkName.includes('hedera')) {
    return hederaPrivateKey ? [hederaPrivateKey] : [];
  }
  
  // Ensure default private key also has 0x prefix if it exists
  let defaultPKToUse = defaultPrivateKey;
  if (defaultPKToUse && !defaultPKToUse.startsWith("0x")) {
    defaultPKToUse = "0x" + defaultPKToUse;
  }
  return defaultPKToUse ? [defaultPKToUse] : [];
}
...
module.exports = {
  solidity: {
    ...
    settings: {
      optimizer: {
        enabled: true,
        runs: 1, // Optimized for size due to contract size limits
      },
      viaIR: true,
      metadata: {
        bytecodeHash: "none",
  ...
  networks: {
    ...
    // Hedera Networks (uses HEDERA_PRIVATE_KEY)
    hedera: {
      url: process.env.HEDERA_RPC_URL || "https://mainnet.hashio.io/api",
      accounts: getAccounts('hedera'),
      chainId: 295,
    ...
    rootstockTestnet: {
      ...
      accounts: getAccounts('rootstockTestnet'),
      chainId: 31,
      gasPrice: 60000000, // 0.06 Gwei (RSK uses lower gas prices)
      timeout: 60000,
    },
